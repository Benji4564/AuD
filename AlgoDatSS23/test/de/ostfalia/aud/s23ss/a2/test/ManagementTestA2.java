package de.ostfalia.aud.s23ss.a2.test;
...
public class ManagementTestA2 {
	...
	private String duplMsg = "Es druerfen keine doppelten Eintraege vorkommen.";
	...
	private static Integer[] allKeysSorted; 
	...
	public void setUp() throws Exception {
		...
		if (expData == null) {			
			...
			allKeysSorted = expData.sort().getKeys().toArray(new Integer[0]);
		}
		if (!evalOperations) {
	 ...
	 *	<li>Erwartet: <br>
	 *		<ul>
	 *			<li>Anzahl der Datensaetze = 0.</li>
	 *			<li>Anzahl Operationen: 1 &plusmn; 1.</li>
	 *		</ul>
	 * </ul>
	 */
	@Test
	 ...
	 *	<li>Erwartet: <br>			
	 *		<ul>
	 *			<li>Datensatz "10796;1959-06-30;Breannda;Billingsley;F;1990-11-08;Management".</li>
	 *			<li>Anzahl Operationen: 1 &plusmn; 1.</li>
	 *		</ul>
	 * </ul>
	 */
	 ...
	 *	<li>Erwartet: <br>
	 *		<ul>
	 *			<li>data[0] bis data[9] in der Mitarbeiterverwaltung.</li>
	 *			<li>Anzahl Operationen: 21 &plusmn; 5.</li>
	 *		</ul>
	 * </ul>
	 */
	@Test
	...
	public void testKonstruktorZehnDatensaetze() {
		...
		evaluate(trace, mgnt, 21, 5);
		evaluate(mgnt, true, allKeysSorted);
	}
	...
	public void testInsertReverse() {
		...
		for (int i = data.length ; i > 0; i--) {
			traceL1.add("Konstruktoraufruf Member(\"%s\").", data[i - 1]);
			IEmployee emp = new Employee(data[i - 1]);
			traceL1.addInfo("Aufruf von insert(%s).", emp);
			mgnt.insert(emp);
		}
		trace.add(traceL1, traceL1.hasOccurrences());
	 ...
	 *      Schluesseln und Mitgliedernamen.</li>
	 *	<li>Erwartet: <br>
	...
	public void testInsertSearch() {
		testEmployee();		
		FunctionalCollector<Integer, IEmployee> collector = 
				new FunctionalCollector<>(traceL2, "%s", (c, o) -> (o != null) ? o.getKey() : c.index());
		collector.define("toString", (obj, c) -> obj.toString());	
		Evaluate eval = new Evaluate(traceL2).occurrenceLimits(MAXVIEW);
		trace.add("Konstruktoraufruf Management().");
		IManagement mgnt = new Management(); 
		
		int idx = 0;
		for (ICollectorItem<Integer, IEmployee> entry : expData) {
			ICollected<Integer, IEmployee> section = expData.section(0, idx + 1);
			trace.add(Evaluate.callConstruktor("employee", Employee.class, entry.getPresentation()));
			IEmployee emp = new Employee(entry.getPresentation());
			traceL1.add("Aufruf von insert(%s).", "employee");
			mgnt.insert(emp);
			
			//---------------------------------------------
			Department dep = Department.create(entry.getPart(6).toString());
			traceL1.add("Aufruf von member(%s).", dep.name());
			collector.clear().useIdentifier("employee", "{", "}");
			ICollected<Integer, IEmployee> got = collector.collectAll(mgnt.members(dep));
			traceL2.add(evalMsg);
			eval.equalsByKeys(section.where((c, it) -> it.getPart(6).equals(dep.toString())), got);
			traceL2.add(PassTrace.ifEquals(duplMsg,	"[]", got.multipleKeys()));
			eval.addToTrace(traceL1);		
			//---------------------------------------------
			int key = entry.getKey();
			traceL1.add("Aufruf von search(%d).", key);
			traceL1.addInfo(PassTrace.ifEquals(evalMsg, entry.getPresentation(), mgnt.search(key)));			
			//---------------------------------------------			
			...
			collector.clear().useIdentifier("employee", "{", "}");
			got = collector.collectAll(mgnt.search(nachname, vorname));
			traceL1.addInfo(evalMsg);
			eval.contains(section.where((c, it) -> it.getPart(2).equals(vorname) && 
					                               it.getPart(3).equals(nachname)), got);
			eval.addToTrace(traceL1);
			//---------------------------------------------
			trace.add(traceL1, traceL1.hasOccurrences());
			idx++;
	...
	public void testInsertSorted() {
		...
		trace.add("Datensaetze in die Mitarbeiterverwaltung einfuegen.");
		for (ICollectorItem<Integer, IEmployee> entry : expData.with(allKeysSorted)) {
			traceL1.add("Konstruktoraufruf Employee(\"%s\").", entry.getPresentation());
			IEmployee emp = new Employee(entry.getPresentation());
			traceL1.addInfo("Aufruf von insert(%s).", emp);
			mgnt.insert(emp);
		}
	 ...
	 *				gefunden werden.</li>
	 *			<li>Die Methode search(int) muss den zugehoerigen Datensatz zurueckliefern.</li>
	 *			<li>Anzahl Operationen: 3 &plusmn; 1.</li>
	 *		</ul> 
	 *	</li>
	 * </ul>
	 */
	 ...
	 *			<li>Research: 0</li>
	 *		</ul> 
	 *		<li>Anzahl Operationen: 3..7 &plusmn; 1.</li>
	 *	</li>
	 * </ul>
	 */
	@Test
	...
	public void testMemberDepartment() {
		...
		got = mgnt.members(d);
		evaluate(got, false, 10942, 10943, 10948);
		
		d = Department.SALES;
		trace.add("Aufruf von member(%s).", d);
		got = mgnt.members(d);
		evaluate(got, false, 10034, 10041);
		
		d = Department.MANPOWER;
		trace.add("Aufruf von member(%s).", d);
		got = mgnt.members(d);
		evaluate(got, false, 10005);
	...
	public void testInstances() {
		...
		trace.separator();
		evaluate(mgnt1, false, data1.getKeys().toArray(new Integer[6]));
		
		trace.add("Instanz mgnt2 muss 4 Datensaetze enthalten.");
		trace.separator();
		evaluate(mgnt2, false, data2.getKeys().toArray(new Integer[4]));
	}
